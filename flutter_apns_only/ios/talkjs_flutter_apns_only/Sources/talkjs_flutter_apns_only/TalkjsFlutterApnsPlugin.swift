...
@objc
public class TalkjsFlutterApnsPlugin:
  NSObject, FlutterPlugin, UNUserNotificationCenterDelegate, FlutterSceneLifeCycleDelegate
{
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    // We continue to be registered to App Delegate to support apps that haven't migrated to UIScene lifecycle.
  ...
  func setNotificationCategories(arguments: Any) {
    ...
    func decodeCategoryOptions(data: [String]) -> UNNotificationCategoryOptions {
      let mapped = data.compactMap { UNNotificationCategoryOptions.stringToValue[$0] }
      return .init(mapped)
    }
    ...
    func decodeActionOptions(data: [String]) -> UNNotificationActionOptions {
      let mapped = data.compactMap { UNNotificationActionOptions.stringToValue[$0] }
      return .init(mapped)
    }
  ...
  // UIScene equivalent to `applicationDidEnterBackground`
  ...
  // UIScene equivalent to `applicationDidBecomeActive`
    ...
    if #available(iOS 16.0, *) {
      Task {
        do {
          ...
          // We don't have the badge permission
        }
      }
    } else {
      UIApplication.shared.applicationIconBadgeNumber = -1
  ...
  // Called when app is both in background and foreground
  public func application(
    _ application: UIApplication,
    ...
    if resumingFromBackground {
      // This code is ported from `didFinishLaunchingWithOptions` after the migration to UIScene lifecycle
